"""
Holzwagen mit GERIGTEN, DREHENDEN Raedern -> animiertes FBX.
=============================================================
ZWECK: Testen, ob Project Zomboid B42 die Animation eines an die Hand
angehefteten Item-Modells (attachment Bip01_Prop1) ueberhaupt abspielt.
Wir bauen einen Wagen, dessen beide Raeder an EINEN Achs-Bone gebunden sind,
und legen eine Dreh-Animation (360 Grad, Loop) darauf. Export als FBX MIT
Armature + Animation.

BEDIENUNG (Blender):
  1. Scripting-Tab -> dieses Skript oeffnen -> "Run Script" (oben).
  2. Es exportiert  ~/HolzwagenExport/holzwagen_t2_anim.fbx  (mit Animation).
  3. Diese FBX nach  HolzwagenMod/42/media/models_X/  kopieren.
  4. Im Item-Script den Mesh-Namen auf  holzwagen_t2_anim  setzen (Test an T2),
     Spiel neu starten, Wagen schieben und schauen, ob die Raeder drehen.

Hinweis: Wenn PZ die Animation NICHT abspielt (Raeder stehen still), ist damit
empirisch bewiesen, dass animierte Hand-Props nicht gehen - dann verwerfen wir
den Ansatz. Wenn sie drehen: wir ziehen es auf alle Modelle.
"""
import bpy, math, os, numpy as np
from mathutils import Vector

# ---------------- aufraeumen ----------------
bpy.ops.object.select_all(action='SELECT')
bpy.ops.object.delete()
for c in list(bpy.data.collections):
    bpy.data.collections.remove(c)

col = bpy.data.collections.new("HolzwagenAnim")
bpy.context.scene.collection.children.link(col)
lc = bpy.context.view_layer.layer_collection.children[col.name]
bpy.context.view_layer.active_layer_collection = lc

# ---------------- Materialien (Atlas-Zonen wie im Hauptskript) ----------------
def mat(name, rgb):
    m = bpy.data.materials.get(name) or bpy.data.materials.new(name)
    m.use_nodes = True
    b = next((n for n in m.node_tree.nodes if n.type == "BSDF_PRINCIPLED"), None)
    if b: b.inputs["Base Color"].default_value = (*rgb, 1.0)
    m.diffuse_color = (*rgb, 1.0)
    return m
WOOD   = mat("Wood",     (0.50, 0.30, 0.14))
WOOD_D = mat("Wood_Dark",(0.34, 0.20, 0.10))
METAL  = mat("Metal",    (0.45, 0.45, 0.50))
TIRE   = mat("Tire",     (0.06, 0.055,0.05))
NAME2Q = {"Wood":0, "Wood_Dark":1, "Metal":2, "Tire":3}

def add(o, name, material):
    o.name = name
    for c in o.users_collection: c.objects.unlink(o)
    col.objects.link(o)
    if o.data.materials: o.data.materials[0] = material
    else: o.data.materials.append(material)
    return o
def box(sx, sy, sz, loc, name, m):
    bpy.ops.mesh.primitive_cube_add(size=1, location=loc)
    o = bpy.context.active_object; o.scale = (sx, sy, sz)
    bpy.ops.object.transform_apply(scale=True)
    return add(o, name, m)
def cyl(r, h, loc, name, m, axis="z", verts=20):
    rot = {"x": (0, math.pi/2, 0), "y": (math.pi/2, 0, 0), "z": (0, 0, 0)}[axis]
    bpy.ops.mesh.primitive_cylinder_add(radius=r, depth=h, location=loc, rotation=rot, vertices=verts)
    o = bpy.context.active_object
    bpy.ops.object.transform_apply(rotation=True, scale=True)
    return add(o, name, m)

# ---------------- Masse ----------------
BED_L,BED_W,BED_T = 1.40,0.86,0.06; WALL_H=0.30
WHEEL_R,WHEEL_W   = 0.34,0.09; HANDLE_L=0.85
Z_AXLE = WHEEL_R; Z_BED = Z_AXLE+0.10
AXLE_HALF = BED_W/2 + WHEEL_W/2 + 0.02   # X-Abstand Achsmitte -> Rad

# ---------------- KOERPER (alles ausser Raedern) ----------------
body_objs = []
body_objs.append(box(BED_W,BED_L,BED_T,(0,0,Z_BED),"Boden",WOOD))
st,gap=0.05,0.045
for i in range(3):
    z=Z_BED+BED_T/2+st/2+i*(st+gap)
    body_objs += [
        box(st,BED_L,st,(-BED_W/2+st/2,0,z),f"LatteL{i}",WOOD_D),
        box(st,BED_L,st,( BED_W/2-st/2,0,z),f"LatteR{i}",WOOD_D),
        box(BED_W,st,st,(0,-BED_L/2+st/2,z),f"LatteV{i}",WOOD_D),
        box(BED_W,st,st,(0, BED_L/2-st/2,z),f"LatteH{i}",WOOD_D),
    ]
for sx in (-1,1):
    for sy in (-1,1):
        body_objs.append(box(0.06,0.06,WALL_H,(sx*(BED_W/2-0.03),sy*(BED_L/2-0.03),
                          Z_BED+BED_T/2+WALL_H/2),"Pfosten",WOOD))
# Achse gehoert zum Koerper (steht still), nur die Raeder drehen
body_objs.append(cyl(0.035,BED_W+0.12,(0,0,Z_AXLE),"Achse",METAL,axis="x",verts=12))
for sx in (-1,1):
    body_objs.append(box(0.05,HANDLE_L,0.05,(sx*0.22,BED_L/2+HANDLE_L/2-0.05,Z_BED),"Holm",WOOD))
body_objs.append(box(0.49,0.05,0.05,(0,BED_L/2+HANDLE_L-0.05,Z_BED),"Griff",WOOD))

# ---------------- RAEDER (separat, drehen) ----------------
wheel_objs = []
for sgn,side in ((-1,"L"),(1,"R")):
    x = sgn*AXLE_HALF
    wheel_objs.append(cyl(WHEEL_R,WHEEL_W,(x,0,Z_AXLE),f"Rad{side}",TIRE,axis="x"))
    wheel_objs.append(cyl(WHEEL_R*0.32,WHEEL_W*1.05,(x,0,Z_AXLE),f"Nabe{side}",WOOD,axis="x",verts=14))
    for k in range(6):
        o=box(WHEEL_W*0.4,0.03,WHEEL_R*1.45,(x,0,Z_AXLE),f"Speiche{side}{k}",WOOD_D)
        o.rotation_euler[0]=k*math.pi/3; bpy.ops.object.transform_apply(rotation=True)
        wheel_objs.append(o)

def join(objs, name):
    bpy.ops.object.select_all(action='DESELECT')
    for o in objs: o.select_set(True)
    bpy.context.view_layer.objects.active = objs[0]
    bpy.ops.object.join()
    j = bpy.context.view_layer.objects.active; j.name = name
    return j

body   = join(body_objs,  "Body")
wheels = join(wheel_objs, "Wheels")

# ---------------- UV: Box-Projektion in die 4 Atlas-Zonen ----------------
def atlas_uv(me):
    if not me.uv_layers: me.uv_layers.new(name="UVMap")
    uvl=me.uv_layers.active.data; loops,verts=me.loops,me.vertices
    raw={}; per_q={0:[],1:[],2:[],3:[]}; poly_q=[]
    for poly in me.polygons:
        q=NAME2Q.get(me.materials[poly.material_index].name,0); poly_q.append(q)
        n=poly.normal; ax=int(np.argmax([abs(n.x),abs(n.y),abs(n.z)]))
        for li in range(poly.loop_start,poly.loop_start+poly.loop_total):
            co=verts[loops[li].vertex_index].co
            uv=(co.y,co.z) if ax==0 else ((co.x,co.z) if ax==1 else (co.x,co.y))
            raw[li]=uv; per_q[q].append(uv)
    bounds={q:(min(p[0] for p in pts),max(p[0] for p in pts),
               min(p[1] for p in pts),max(p[1] for p in pts)) for q,pts in per_q.items() if pts}
    M=0.012
    for pi,poly in enumerate(me.polygons):
        q=poly_q[pi]; u0,u1,v0,v1=bounds[q]; c=q%2; r=q//2
        for li in range(poly.loop_start,poly.loop_start+poly.loop_total):
            ru,rv=raw[li]
            nu=(ru-u0)/(u1-u0+1e-9); nv=(rv-v0)/(v1-v0+1e-9)
            uvl[li].uv=((c*0.5)+M+nu*(0.5-2*M),(r*0.5)+M+nv*(0.5-2*M))
atlas_uv(body.data); atlas_uv(wheels.data)

# ---------------- ein Atlas-Material an beide Meshes ----------------
home=os.path.expanduser("~"); outdir=os.path.join(home,"HolzwagenExport")
os.makedirs(outdir,exist_ok=True)
tex_path=os.path.join(outdir,"holzwagen_tex.png")
fin=bpy.data.materials.new("Holzwagen_Mat"); fin.use_nodes=True
nt=fin.node_tree
for n in list(nt.nodes): nt.nodes.remove(n)
out=nt.nodes.new("ShaderNodeOutputMaterial"); out.location=(300,0)
bsdf=nt.nodes.new("ShaderNodeBsdfPrincipled"); bsdf.location=(0,0)
tex=nt.nodes.new("ShaderNodeTexImage"); tex.location=(-400,0); tex.interpolation='Linear'
if os.path.exists(tex_path): tex.image=bpy.data.images.load(tex_path, check_existing=True)
nt.links.new(bsdf.inputs["Base Color"], tex.outputs["Color"])
nt.links.new(out.inputs["Surface"], bsdf.outputs["BSDF"])
for ob in (body, wheels):
    ob.data.materials.clear(); ob.data.materials.append(fin)

# ---------------- ARMATURE: Root + Wheel-Bone ----------------
bpy.ops.object.armature_add(enter_editmode=True, location=(0,0,0))
arm = bpy.context.active_object; arm.name="HolzwagenRig"
eb = arm.data.edit_bones
root = eb[0]; root.name="Root"; root.head=(0,0,0); root.tail=(0,0.2,0)
# Wheel-Bone entlang der Achse (X). Lokale Y-Achse zeigt dann nach +X (Achsachse)
# -> Rotation um lokale Y dreht die Raeder um die Achse.
wb = eb.new("Wheel")
wb.head=(-AXLE_HALF, 0, Z_AXLE); wb.tail=(AXLE_HALF, 0, Z_AXLE); wb.parent=root
bpy.ops.object.mode_set(mode='OBJECT')

# ---------------- Meshes an Bones binden (volle Gewichtung) ----------------
def bind(ob, bone_name):
    vg = ob.vertex_groups.new(name=bone_name)
    vg.add(range(len(ob.data.vertices)), 1.0, 'REPLACE')
    m = ob.modifiers.new(name="Armature", type='ARMATURE'); m.object=arm
    ob.parent = arm
bind(body,   "Root")
bind(wheels, "Wheel")

# ---------------- Dreh-Animation auf den Wheel-Bone (Loop) ----------------
FPS=30; FRAMES=24                      # 24 Frames = sauberer 360-Grad-Loop
bpy.context.scene.render.fps=FPS
bpy.context.scene.frame_start=1
bpy.context.scene.frame_end=FRAMES
bpy.ops.object.mode_set(mode='POSE')
pb = arm.pose.bones["Wheel"]
pb.rotation_mode='XYZ'
def key(frame, deg):
    bpy.context.scene.frame_set(frame)
    pb.rotation_euler=(0.0, math.radians(deg), 0.0)   # um lokale Y = Achsachse
    pb.keyframe_insert(data_path="rotation_euler", frame=frame)
key(1, 0); key(FRAMES+1, 360)         # 0 -> 360 Grad
# lineare Interpolation, damit es gleichmaessig dreht
act = arm.animation_data.action
for fc in act.fcurves:
    for kp in fc.keyframe_points: kp.interpolation='LINEAR'
arm.animation_data.action.name="WheelSpin"
bpy.ops.object.mode_set(mode='OBJECT')

# ---------------- Export: FBX MIT Armature + Animation ----------------
bpy.ops.object.select_all(action='DESELECT')
for ob in (arm, body, wheels): ob.select_set(True)
bpy.context.view_layer.objects.active = arm
fbx=os.path.join(outdir,"holzwagen_t2_anim.fbx")
bpy.ops.export_scene.fbx(
    filepath=fbx, use_selection=True, apply_unit_scale=True,
    object_types={'MESH','ARMATURE'}, add_leaf_bones=False,
    bake_anim=True, bake_anim_use_all_actions=False,
    bake_anim_force_startend_keying=True, bake_anim_step=1.0,
)
print("Animiertes Modell exportiert:", fbx)
print("-> nach HolzwagenMod/42/media/models_X/ kopieren und im Item-Script testen.")
